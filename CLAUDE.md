...
- **Ключ `Idempotency-Key` глобален для сервиса**, не привязан к сессии и к API-ключу; повтор принимается только при совпадении `session_id`, `harness`, `tool` и `raw`. Привязка к аутентифицированному ключу — отдельная задача вместе с атрибуцией решения к ключу.
