...
- **Ключ `Idempotency-Key` глобален для сервиса**, не привязан к сессии и к API-ключу; повтор принимается только при совпадении `session_id`, `harness`, `tool`, `raw`, `cwd` и `profile_id`. Привязка к аутентифицированному ключу — отдельная задача вместе с атрибуцией решения к ключу.
