...
- **Повтор ключа после истечения TTL** (24 ч) даёт новое решение, но его строка не попадает в базу из-за уникального индекса по паре `(principal, idempotency_key)` (с v3.2; до неё — по одному столбцу); writer пишет предупреждение.
...
- **`downgrade` миграции `0007_key_attribution` сужает уникальность обратно к одному столбцу.** Если после апгрейда два разных принципала успели записать один и тот же `Idempotency-Key`, откат до `0006` упадёт на создании индекса — строки нужно де-дуплицировать вручную до отката. Обычная цена сужающего индекса, не дефект миграции.
- **`Idempotency-Key` по-прежнему не привязан к сессии.** С v3.2 он в границах принципала (`key_id` либо статический токен), но два разных вызова инструмента в одной сессии, случайно пославшие одну и ту же строку под одним и тем же ключом, всё ещё столкнутся — привязка к сессии не входила в эту задачу.
