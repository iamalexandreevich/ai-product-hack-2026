...
- Дорожная карта версий (v1→v5): `docs/superpowers/service/specs/context-versions-roadmap.md`. v1–v4 идут по оси «сколько контекста видит сервис», v5 — по оси независимости от провайдера модели. **Реализован v1** (одно действие + последнее сообщение пользователя; форма кода — v1.5, карта ниже), **поверх него v2**:
  ...
  **Реализован v3**: спека `docs/superpowers/service/specs/2026-09-04-agentgate-v3-rules-and-inspect-design.md`, план `docs/superpowers/service/plans/2026-09-04-agentgate-v3-rules-and-inspect.md`, отчёт `docs/reports/task-20-v3-rules-and-inspect.md`. Добавлены `rules` и `call_id` в `DecideRequest` и маршрут `POST /v1/inspect` (детекторы → маска/`drop` → классификатор по флагу). Закрытый список содержимого промпта inspect: системный промпт, профиль, prose-слоты, `[TASK]`, `[HISTORY]` (только при непустом диалоге), `[PROVENANCE]`, `[FLAGS]`, `[OUTPUT]` — всё через `render.j()`, `metadata` и рассуждения агента — никогда. Расширять только вместе с docstring `inspect/classify.py` и этим файлом.
...
| `inspect/` | Каскад inspect. `detectors.py` — `Detector`, `Action`, `Finding`, таблица детекторов (`INJECTION`, `PIPE_EXEC`, `ENCODED`, `INVISIBLE`) и `scan`; `hints`/`precheck` — дешёвые предпроверки перед регулярками. `chain.py` — `INSPECT_STAGE1`, порядок детекторов. `mask.py` — `apply`: построчная замена и порог `drop` (более половины строк). `classify.py` — `InspectCase`, протокол `InspectClassifier`, `LLMInspectClassifier`, `InspectOutcome`. |
...
| `store/` | `writer.py` — `DecisionWriter` (Protocol) и три реализации, `protocols.py` — `Stored` (то, что writer'у нужно от исхода, будь то `Decision` или `Inspection`: `to_record`, `allow_cache_entry`, `session_state`, `session_ref`), `repo.py`, `models.py`, `mapper.py` (единственное место, знающее про `metadata_`), `keys.py`, `db.py`. |
