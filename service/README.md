...
Строка в таблице `NEEDLE_FORMS` (`agentgate/inspect/secrets.py`): подсказки-подстроки, регулярка с группой `value` и, если нужно, проверка совпадения. Значение в группе `value` заменяется на `[gate: secret redacted]`, остальная строка остаётся. Подсказки должны быть не строже регулярки: строка, которую регулярка совпадёт, обязана содержать одну из подсказок.

```python
Form(("hf_",), re.compile(r"(?P<value>\bhf_[A-Za-z0-9]{30,}\b)")),
```

Корпус `tests/inspect/fixtures/secret_false_positives.txt` — контракт на ложные срабатывания: после новой формы он должен проходить без единой редакции, а восемь латентных корпусов в `tests/inspect/test_secrets.py` — оставаться под 5 мс p50.
...
Секция `inspect` управляет проверкой результатов инструментов (`POST /v1/inspect`):
...
С v3.2 (миграция `migrations/versions/0007_key_attribution.py`) решение, принятое под выданным ключом, несёт его `key_id` в строке Postgres и в JSONL; фильтр ленты — `GET /v1/decisions?key_id=<ULID>`. Та же миграция переносит уникальность `Idempotency-Key` со столбца `idempotency_key` на пару `(principal, idempotency_key)` — повтор с одинаковой строкой у двух разных предъявителей (двух ключей, либо ключа и статического токена) больше не сталкивается. `downgrade` сужает индекс обратно к одному столбцу: если после апгрейда два принципала успели записать один и тот же `Idempotency-Key`, откат потребует ручной чистки строк перед тем, как `downgrade` дойдёт до `0006`.

С v3.3 добавились две миграции. `0008_key_id_shape` вешает два `CHECK` — `ck_api_keys_id_ulid` на `api_keys.id` и `ck_decisions_key_id_ulid` на `decisions.key_id` (`NULL` разрешён) — по паттерну ULID `^[0-9A-HJKMNP-TV-Z]{26}$`. Так база, а не договорённость, гарантирует, что `key_id` структурно не может оказаться равен строке `token`, которой обозначается статический токен. `upgrade` **намеренно падает**, если в исторических данных есть строка с `key_id` вне этой формы: такой id сервис никогда не выпускал, значит кто-то писал в базу мимо сервиса, и молчаливая правка уничтожила бы след инцидента — правильный ответ оператору — остановка и список строк. `downgrade` безопасен: он только снимает ограничения.

`0009_session_idempotency` заменяет уникальный частичный индекс по паре `(principal, idempotency_key)` на тройку `(principal, session_id, idempotency_key)` с `NULLS NOT DISTINCT` (Postgres 15+; `session_id` nullable, и без этого флага бессессионные вызовы перестали бы конкурировать за уникальность вовсе). `downgrade` сужающий и несёт ту же опасность, что откат `0007`: если после апгрейда один принципал успел записать один и тот же `Idempotency-Key` в двух сессиях, эти строки придётся де-дуплицировать вручную, иначе создание узкого индекса не пройдёт.
