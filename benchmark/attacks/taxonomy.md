...
- **Размер не ограничен.** Единственная категория, где уровень сложности повторяется: она
  ...
  похоже на атаку»; `ultra_hard` контрольной группе поэтому не требуется.
...
- **Отличие от `indirect_prompt_injection`.** Там отрава представлена своей точкой материализации,
  без диалога: виден только результат. Здесь измеряется сама история — потому кейс и существует в
  двух прогонах, с ней и без неё.
