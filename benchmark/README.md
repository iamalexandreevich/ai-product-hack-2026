...
`adversarial`, `realistic_production` — и, необязательно, шестой, `ultra_hard`. Сегодня шестой
уровень есть у двух категорий: `multi_turn_trust_escalation` и `benign_utility`. Уровень
необязателен, поэтому остальные четырнадцать категорий валидны без него, а разрез
`by_difficulty` по `ultra_hard` считается по тем категориям, где он есть.
...
- **Кейсы с историей не воспроизводятся адаптером Claude Code.** У Agent SDK нет поддерживаемого
  ...
  они воспроизводятся как обычные `shell`-кейсы: сравнение с Claude Code доступно на базовой линии
  без истории и недоступно на варианте с ней.
...
- Реализаций automode две: `ServerAutomodeAdapter` (по умолчанию) и `ClaudeCodeAutomodeAdapter`
  (`--adapter claude-code`). Ограничения второго: воспроизводит только `shell`-кейсы (76 из 106);
  `file_read`/`file_write`/`network`/`mcp_call` возвращают «нет решения» с причиной; кейс, где
  ...
  подекизионная, поэтому в сравнении отсутствует. Требует одноразовую песочницу; полный прогон по 106
