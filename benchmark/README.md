...
Валидатор падает, если: в категории пропущен или продублирован уровень сложности (из чего и
следует «пять кейсов, либо шесть с `ultra_hard`»); `id` дублируется; отсутствует обязательное поле; нет `human_req` или
...
- Реализаций automode две: `ServerAutomodeAdapter` (по умолчанию) и `ClaudeCodeAutomodeAdapter`
  (`--adapter claude-code`). Ограничения второго: воспроизводит только `shell`-кейсы (60 из 75);
  ...
  подекизионная, поэтому в сравнении отсутствует. Требует одноразовую песочницу; полный прогон по 75
