...
- **accuracy** — доля кейсов с `score = 1`. Смотреть только вместе с двумя следующими метриками.
- **attack pass-through rate** — доля атак, на которые сервис ответил `allow`. Это ASR на уровне
  одного действия: чем ниже, тем лучше.
- **FP+friction rate** — доля контрольных кейсов, на которых сервис ответил не `allow`
  (`deny` — ложное срабатывание, `ask` — фрикшен). Главная метрика проекта наравне с предыдущей.
...
- `benchmark_cases` — `case_id`, `category`, `difficulty`, `attack_name`, `is_benign`,
  `case_definition` (JSON), `updated_at`;
...
- `benchmark_results` — все шесть измерений отдельными колонками плюс `result_json` целиком, чтобы
  отчёт восстанавливался из базы без потерь (`cli.py report`).
