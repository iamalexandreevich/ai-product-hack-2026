...
export async function install(options: InstallOptions = {}): Promise<InstallPlan[]> {
  ...
  log(`openmagi mode: auto — check with: openmagi status`)
  return applied
}
...
export function uninstall(options: InstallOptions = {}): void {
  ...
  log("openmagi uninstalled; .bak files and state.json were left in place")
  removeBundles(paths)
  // Both names: an install from before the rename put `gate` here.
  fs.rmSync(path.join(paths.binDir, "openmagi"), { force: true })
...
/**
 * An `openmagi` command on PATH. Without it the only way to switch modes
 * outside a TUI is spelling out the installer's entry point, which nobody
 * remembers.
 *
 * The harness wrappers keep their `-gate` suffix on purpose: `kilo-gate` says
 * what it is next to `kilo`, which `kilo-openmagi` would not.
 */
function writeGateCli(paths: GatePaths): void {
  const entry = path.resolve(fileURLToPath(import.meta.url), "..", "..", "bin", "openmagi.js")
  if (!fs.existsSync(entry)) return
  fs.mkdirSync(paths.binDir, { recursive: true })
  // An install that used the old name leaves a `gate` shim pointing at whatever
  // checkout wrote it. Two commands doing the same thing, one of them stale, is
  // worse than one -- so the old name goes when the new one is written.
  fs.rmSync(path.join(paths.binDir, "gate"), { force: true })
  const file = path.join(paths.binDir, "openmagi")
  fs.writeFileSync(
    file,
    ["#!/bin/sh", "# Generated by OPENMAGI.", `exec node '${entry}' "$@"`, ""].join("\n"),
  )
