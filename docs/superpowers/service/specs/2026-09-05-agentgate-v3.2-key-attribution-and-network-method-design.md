...
### 5.5. Клиентские правила и `tool: network`

Ревьюер нашёл дыру, обнаруженную по HTTP: `canonical_units` (`rules/client_rules.py`) не знала о `tool: network` и возвращала `([], [])`, а `_paths` для сетевого действия тоже пуста, — пользовательские `rules` на такое действие не срабатывали никогда. При включённом `trusted_allows` и объявленном `github.com` GET получал `allow/profile.domain-trusted`, даже если `rules: {deny: ["*"]}` — прямое нарушение инварианта «клиентские правила поднимают пол» (корневой `CLAUDE.md`). Исправлено зеркально ветке MCP: канонические единицы сетевого действия — это его домены, каждый сам по себе (`action.domains`, уже приведены к нижнему регистру нормализатором), возвращённые и как `units`, и как `singles`. `deny`/`ask` срабатывают, если совпал хотя бы один домен; `client.allow` требует совпадения каждого — то же обещание «вся строка», что и у MCP-имени. `is_path_pattern` не тронута: шаблон с `/` (например, `github.com/org`) остаётся путевым и никогда не совпадёт с сетевым действием, у которого нет путей — тот же случай, что и с именем MCP-сервера с `/` (спека v3.1 §4.1), задокументированный отдельным пунктом ограничений.
