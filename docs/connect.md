...
`rules.ask` — просьба подтвердить, а не разрешение: с v3.1 она задаёт пол строгости и не даёт решению стать мягче `ask`, но не отменяет `deny` сервиса. Команда из вашего `ask`-списка, которую ступень 2 считает опасной, вернётся как `deny`, а не как `ask`.
