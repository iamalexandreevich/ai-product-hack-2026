...
`Idempotency-Key` живёт в границах вашего API-ключа: одинаковая строка у двух интеграторов больше не сталкивается — раньше повтор был глобален для сервиса, и совпавшая строка отдавала чужое решение. Требование к самому ключу осталось прежним: он должен быть уникален на вызов инструмента.
