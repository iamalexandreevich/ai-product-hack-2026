...
> 2026. Он показывает, какие идентичности git касались каких каталогов, и ничего не говорит
> о величине вклада: работа, не попадающая в коммиты, здесь не видна по определению.

**Идентичности авторов в истории git** (`git log --format='%an <%ae>'`): `Александр`,
`Timur Polishchuk`, `alexey`, `alex-andreevich` (последняя — только первый коммит
репозитория). **Соотнести эти идентичности с полными именами выше по репозиторию нельзя** —
это знание команды, а не факт из git.

**Каталоги, которых касалась каждая идентичность** (по числу коммитов, затронувших каталог;
цифры приведены только чтобы показать, где сосредоточена работа, а не чтобы кого-то ранжировать):

| Каталог | `Александр` | `alexey` | `Timur Polishchuk` |
|---|---|---|---|
| `service/` | 198 | — | — |
| `contracts/` | 50 | — | — |
| `adapters/` | 1 | 2 | 1 |
...
направления действительно разошлись по трём людям: сервис решений и контракты сосредоточены у
одной идентичности, слой интеграции с харнессами — у другой (два коммита, но это весь
TypeScript-монорепозиторий: ядро, шесть плагинов, инсталлер и документация), бенчмарк и
проектные артефакты — у третьей.
...
Кто именно проводил опрос, из репозитория не следует — это по-прежнему командный контекст.
