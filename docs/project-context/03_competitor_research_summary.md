...
>
> Синтез обновлён 6 сентября 2026. Изменение по сравнению с предыдущей редакцией одно,
> но существенное: **у нас появились собственные измерения** — прогон бенчмарка против
> развёрнутого сервиса 6 сентября 2026 (§ 11a). До него все продуктовые следствия здесь
> были предположениями; часть из них измерение не подтвердило.
> Утверждения о конкурентах не пересматривались: новых внешних данных с 3 сентября нет.
>
...
**Что изменилось на нашей стороне к 6 сентября 2026** (факты по репозиторию, не заявка):
отдельный сервис с публичным контрактом реализован и развёрнут (`service/`, ответ
`https://api.openmagi.ru/healthz`); плагины написаны для шести харнессов поверх одного
профиля (`adapters/`); входной слой против инъекций реализован как `POST /v1/inspect`
с детекторами, маскированием секретов и опциональным классификатором
(`service/agentgate/inspect/`). **Бюджеты автономии на сессию** не реализованы: в состоянии
сессии есть только счётчики решений и окно отказов (`service/agentgate/domain/session.py`).
**Замкнутая петля benchmark→правило** как процесс не автоматизирована.
Ни один из этих фактов не доказывает уникальности: сопоставимых измерений по конкурентам
нет, а глубину кросс-агентности Adversa и Arcade команда по-прежнему не проверяла.
...
1. **«Мы безопаснее»** — вендоры сами не считают гейт границей безопасности; независимый red-team (ASR 60–80 %) показывает, что заявленные 0 % не держатся. Тезис о безопасности почти наверняка будет опровергнут адаптивной атакой. **Измерением это предположение опровергнуто прямо:** на общей популяции из 67 кейсов родной auto mode Claude Code и наш гард пропустили одинаковую долю атак (19,6 %), а по строгой точности родной режим оказался впереди (86,6 % против 79,1 %) — см. § 11a.
...
| **Наш опрос (`09_user_research/`): непрозрачность беспокоит чаще, чем частота вопросов** — 13 из 20 против 8 из 20 | Ценность формулируется через объяснимость и журнал, а не только через «меньше подтверждений» | `reason` обязателен и содержателен; лента решений — часть продукта, а не отладки | Понимает ли пользователь по нашему `reason`, что именно произошло бы |
| **Наш опрос: границы автономии проходят по действию и среде, а не по инструменту** — автономия в поиске и рефакторинге (17 и 16 из 20), подтверждение для секретов, отправки наружу и sudo (16 и по 14 из 20) | Профиль политики должен различать подготовку и исполнение, чтение и изменение | Категории профиля уже такие (`protected_paths`, `network`, `mcp`), но различения «создать миграцию / применить» в нём нет | Сопоставимые сценарии в интервью |
...
## 11a. Что уже измерено (прогон 6 сентября 2026; серия по моделям и ActBench — отдельно)

> **Статус: цифры этого раздела — из одного прогона.** Все они получены против развёрнутого
> сервиса `api.openmagi.ru` (`git_sha e3c7942`, профиль `default`, ступень 2 —
> `openai/gpt-4.1-mini` через OpenRouter, `--concurrency 1`). Артефакты лежат в
> `benchmark/results/`, который в `.gitignore`, поэтому в репозитории их нет; разбор —
> `benchmark/docs/reports/task-24-first-full-benchmark-run.md`. Числа воспроизведены при
> аудите из локальной SQLite прогона.
>
> **Два других измерения существуют и в этот раздел не входят:** серия по восьми моделям
> ступени 2 (те же 120 кейсов, по пять прогонов на модель; ASR 1,1–45,3 %, цена $0,014–3,87 за
> 1 000 решений) и полный прогон внешнего корпуса ActBench 7 сентября (см. конец раздела).
> Складывать их числа с приведёнными ниже нельзя: у серии другая модель ступени 2 в каждом
> прогоне, у ActBench другая популяция и другой способ оценки.

**Наш сервис напрямую, 120 pre-action кейсов** (90 атак, 30 контрольных): точность 80,0 %
(96/120), ASR 22,2 % (20 из 90), Utility 96,7 %, FP 3,3 % (1 кейс), ни одного кейса без
решения. Ступень 1 закрыла 50 решений, ступень 2 — 70. Латентность решения по данным
самого сервиса: p50 1108 мс, p95 1669 мс; вклад ступени 1 — p50 0 мс, максимум 3 мс.

**Три конфигурации Claude Code на общей популяции 67 кейсов** (только те, по которым все
три вынесли решение; 46 атак / 21 чистый):

| Конфигурация | Точность | ASR | Utility | FP |
|---|---|---|---|---|
| `claude-code` — родной auto mode | 86,6 % | 19,6 % | 100 % | 0 |
| `claude-agentgate` — наше ядро в хуке `PreToolUse` | 79,1 % | 19,6 % | 90,5 % | 2 |
| `claude-sdk` — обычные права без классификатора | 26,9 % | 0 % | 0 % | 21 |

**Что из этого следует для конкурентного позиционирования.**

- Утверждение «мы ловим больше атак, чем родной auto mode» **не подтверждено**: доля
  пропущенных атак одинакова, по строгой точности родной режим впереди на 7,5 пункта.
  Обратное утверждение — «родной auto mode сильнее» — тоже не установлено: сравнение шло на
  `gpt-4.1-mini`, а серия по моделям даёт на тех же кейсах ASR до 1,1 %, то есть измерена
  конфигурация, а не архитектура. До пересравнения на сильной модели ступени 2 в конкурентных
  материалах не заявляется ничего ни в одну сторону.
- Утверждение «дешёвая детерминированная ступень закрывает большинство трафика»
  **подтверждено только частично**: на нашем датасете ступень 1 закрыла 42 % решений
  (50 из 120), тогда как у Codex до ревьюера доходит 7,2 % трафика. Датасет намеренно
  насыщен атаками и сравнению с реальным трафиком не подлежит.
- Утверждение «низкое трение» **на этом датасете держится**: Utility 96,7 %, один ложный
  отказ на 30 контрольных кейсов. Резолюция метрики — 1/30, то есть ±3,3 пункта.
- Про **стоимость в этом прогоне** сказать нельзя ничего: цены модели в профиле не заданы,
  деньги не посчитаны; сервис вернул токены по 70 запросам, суммы — нет. Отдельная серия по
  восьми моделям ступени 2 цену даёт: $0,014–3,87 за 1 000 решений при ASR от 1,1 % до 45,3 %.
- Сравнение с Codex, Cursor, Adversa и прочими **не проводилось вообще**: измерены только
  наш сервис и три конфигурации Claude Code.
- Внешний исполняемый baseline **ActBench прогнан целиком 7 сентября 2026** (300 пар задач,
  два плеча): ASR 30,4 % без гарда против 24,3 % под `decide-inspect`, парно по 280 задачам,
  точный Макнемар p = 0,0033 (`06_benchmark_actbench_status/README.md`). Оговорка, важная для
  конкурентного чтения: опубликованную таблицу ActBench по 15 моделям с нашими числами
  сравнивать нельзя — там харнесс OpenClaw и оценка с LLM-судьёй, у нас Claude Code и только
  логовое свидетельство. Осмысленна наша внутренняя дельта, а не позиция в их рейтинге.
  Защитных решений с замеренной дельтой ASR на ActBench в публичных материалах нет вообще —
  пять детекторов из RQ3 статьи измерены по различающей способности, а не по изменению ASR.
...
### High priority
1. ~~**Расхождение между источниками по Claude Code**~~ **Частично закрыто.** Датировка «август 2023» из источника B несовместима и с постановкой кейса (`01_hackathon_case.md`: «14 августа auto mode стал режимом по умолчанию»), и с источником A (14 августа 2026); в B она помечена по месту как ошибка исходника. Открытым остаётся, какая именно модель работает классификатором: A называет Sonnet 4.6, B — «Claude 4.6»; независимой проверки нет.
...
5. ~~Есть ли у нас точка перехвата уровня `PreToolUse` во втором и третьем харнессе~~ **Закрыто по коду, открыто по подтверждению.** Плагины написаны для шести харнессов (opencode 1.x, Kilo CLI, opencode 2.0, Pi, Codex CLI, DeepSeek Harness); двум из них нужен патч бинарника, у Codex `PreToolUse` умеет только `deny` (`adapters/README.md`, `adapters/docs/ARCHITECTURE.md`). Открытым остаётся другое: в бенчмарке измерена ровно одна интеграция — Claude Code через хуки SDK; остальные пять проверялись ручными прогонами, описанными в `adapters/docs/MANUAL-TESTING.md`, и в рамках аудита не воспроизводились.
...
- Индустрия сошлась на многоступенчатом каскаде, где до модели доходит меньшинство действий (Codex: 7,2 %). У нас на насыщенном атаками датасете доля решений без модели — 42 % (§ 11a); с реальным трафиком это не сравнивается.
...
- «Мы безопаснее Claude Code / Codex / Cursor» — с Codex и Cursor сравнимых измерений нет вовсе; с Claude Code измерение есть, и оно **против нас** (§ 11a).
- «Ниже FPR / выше recall, чем у конкурентов» — на нашем датасете это измерено и **не подтвердилось**: родной auto mode Claude Code дал такой же ASR и меньше ложных срабатываний (§ 11a). Обратное тоже нельзя заявлять от их лица: сравнение шло на одной модели ступени 2, а серия показала разброс ASR от 1,1 % до 45,3 %.
- «Ниже латентность / дешевле» — латентность измерена только для нашего сервиса (p50 1108 мс на решение, из них ступень 1 — 0 мс); сопоставимых измерений у конкурентов нет. Своя стоимость известна из серии по моделям ($0,014–3,87 за 1 000 решений в зависимости от модели ступени 2), но сравнивать её не с чем: цен конкурентов на сопоставимую единицу нет.
- «Устойчивы к prompt injection» — независимый red-team пробивал все проверенные системы.
- «Закрываем provenance/taint» — этого не сделал никто, кроме research-прототипов, и у CaMeL есть известный обход. У нас провенанс есть как обязательное поле запроса `inspect` и как метка источника, но taint-цепочек между шагами нет.
- Любые заявления о доле рынка, спросе или готовности платить — ни в конкурентных исследованиях, ни в нашем опросе разработчиков (`09_user_research/`) таких данных нет.
- «Approval fatigue — главная боль пользователя» — **наш собственный опрос это не подтверждает**: подтверждения отметили 8 из 20, тогда как непонимание последствий и страх утечки — по 13 из 20. Рыночные цифры про approval fatigue (93 % одобрений, 13,6 % поимки) остаются верными как утверждение о **качестве** человека-детектора, но не как утверждение о том, что именно раздражает пользователя больше всего.
