...
## 11a. Что уже измерено (прогон 6 сентября 2026)

> **Статус: одно измерение, не серия.** Все цифры — из одного прогона против развёрнутого
...
- Про **стоимость** сказать нельзя ничего: цены модели в профиле не заданы, поэтому
  деньги не посчитаны; сервис вернул токены по 70 запросам, суммы — нет.
...
- Внешний исполняемый baseline **ActBench подключён кодом, но не запускался** — нативному
  оценщику нужен ключ модели, которого на машине прогона не было (`benchmark/baselines/`).
...
- «Ниже FPR / выше recall, чем у конкурентов» — на нашем датасете это измерено и **не подтвердилось**: родной auto mode Claude Code дал такой же ASR и меньше ложных срабатываний (§ 11a).
- «Ниже латентность / дешевле» — латентность измерена только для нашего сервиса (p50 1108 мс на решение, из них ступень 1 — 0 мс); сопоставимых измерений у конкурентов нет. Стоимость не измерена вовсе: цены модели в профиле не заданы.
